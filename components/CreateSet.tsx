...
const CreateSet: React.FC<CreateSetProps> = ({ onSave, onCancel, onGoToAiTextbook, history = [], onSelectHistory }) => {
  ...
  const [status, setStatus] = useState<string>('ACTIVE'); // ACTIVE, DRAFT
  ...
  const handleSelectServerSet = async (setId: number | string) => {
      ...
      try {
          ...
          if (response.code === 1000) {
              ...
              setStatus(apiData.status || 'ACTIVE');
              setPrivacy(apiData.privacy || 'PUBLIC');
                              ...
                              serverSets.map((record) => (
                                  <tr key={record.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                      ...
                                      <td className="px-6 py-4">
                                          <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${record.status === 'DRAFT' ? 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-400' : 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400'}`}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
            <div className="mx-auto w-full max-w-[1920px] grid grid-cols-1 lg:grid-cols-4 gap-8 relative h-full">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 transition-colors">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1 flex items-center gap-1"><FileEdit size={10} /> Trạng thái</label>
                                    <select 
                                      value={status}
                ...
                <div className="lg:col-span-3 space-y-6 h-full flex flex-col">
                    {/* Toolbar and Editor Content (Bản gốc) */}
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex-shrink-0">
                        <div className="flex items-center gap-4 w-full sm:w-auto overflow-x-auto">
                            <button onClick={() => handleSwitchMode('VISUAL')} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm whitespace-nowrap transition-colors ${editorMode === 'VISUAL' ? 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 dark:text-gray-400'}`}><Layers size={16} /> {t('create_set.visual_mode')}</button>
                            <button onClick={() => handleSwitchMode('TEXT')} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm whitespace-nowrap transition-colors ${editorMode === 'TEXT' ? 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 dark:text-gray-400'}`}><FileText size={16} /> {t('create_set.text_mode')}</button>
                            <button onClick={() => setShowTextGuide(true)} className="flex items-center gap-1 text-gray-500 hover:text-indigo-600 dark:text-gray-400 dark:hover:text-indigo-400 text-sm font-medium transition-colors whitespace-nowrap"><HelpCircle size={16} /> {t('create_set.guide')}</button>
                        </div>
                        <div className="flex gap-2">
                            <button onClick={() => openAiModal('TEXT_TOPIC')} disabled={!canAiTopic} className={`px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2 shadow-sm transition-all ${canAiTopic ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white hover:opacity-90' : 'bg-gray-200 dark:bg-gray-700 text-gray-400 cursor-not-allowed grayscale'}`}>{canAiTopic ? <Sparkles size={16} /> : <Lock size={14} />} {t('create_set.use_ai')}</button>
                        </div>
                    </div>
                    ...
                    {editorMode === 'VISUAL' ? (
                        <div className="space-y-6 flex-1">
                          ...
                          <div key={card.id} ref={(el) => { cardRefs.current[card.id] = el; }} className={`bg-white dark:bg-gray-800 rounded-xl shadow-sm border transition-all duration-300 overflow-hidden relative group scroll-mt-24 ${activeCardId === card.id ? 'border-indigo-500 ring-1 ring-indigo-200 dark:ring-indigo-900 shadow-md' : 'border-gray-200 dark:border-gray-700 hover:border-indigo-300 dark:hover:border-indigo-500'}`} onClick={() => setActiveCardId(card.id)}>
                              <div className="px-6 py-4 bg-gray-50/50 dark:bg-gray-700/30 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                                  <div className="flex items-center gap-3">
                                      <span className="bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-200 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm shadow-sm">{index + 1}</span>
                                      ...
                                      {(!card.definition || !card.options?.includes(card.definition)) && (
                                          <span className="text-orange-500 text-xs font-bold flex items-center gap-1"><AlertCircle size={14} /> {t('create_set.choose_correct')}</span>
                                      )}
                                  </div>
                                  <button onClick={(e) => { e.stopPropagation(); handleRemoveCard(card.id); }} className="text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700" title={t('create_set.delete_card')}><Trash2 size={18} /></button>
                              </div>
                              <div className="p-6">
                                  <div className="mb-6"><label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2 flex items-center gap-1"><BrainCircuit size={14} /> {t('create_set.question_label')}</label><textarea className="w-full p-4 rounded-xl bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 focus:bg-white dark:focus:bg-gray-700 focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-900 outline-none transition-all text-gray-900 dark:text-white placeholder-gray-400 font-medium text-lg leading-relaxed resize-none" placeholder={t('create_set.question_ph')} rows={2} value={card.term} onChange={e => handleTermChange(card.id, e.target.value)}/></div>
                                  <div className="mb-6"><label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-3 flex items-center gap-1"><List size={14} /> {t('create_set.options_label')}</label><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{card.options?.map((option, optIdx) => { const isCorrect = card.definition === option && option !== ''; return (<div key={optIdx} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${isCorrect ? 'border-green-500 bg-green-50 dark:bg-green-900/20 shadow-sm' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 focus-within:border-gray-400 dark:focus-within:border-gray-500'}`}><div onClick={() => handleSetCorrectAnswer(card.id, option)} className={`w-6 h-6 rounded-full border-2 flex items-center justify-center cursor-pointer flex-shrink-0 transition-all ${isCorrect ? 'border-green-500 bg-green-500 text-white scale-110' : 'border-gray-300 dark:border-gray-500 text-transparent hover:border-green-400'}`} title={t('create_set.choose_correct')}><Check size={14} strokeWidth={4} /></div><div className="flex-1"><input type="text" className="w-full bg-transparent outline-none text-gray-800 dark:text-gray-200 placeholder-gray-400 font-medium text-sm" placeholder={t('create_set.option_ph', { char: String.fromCharCode(65 + optIdx) })} value={option} onChange={(e) => handleOptionChange(card.id, optIdx, e.target.value)}/></div><button onClick={() => handleRemoveOption(card.id, optIdx)} className="text-gray-300 hover:text-red-500 transition-colors p-1"><X size={14} /></button></div>);})}</div>{(card.options?.length || 0) < 6 && (<button onClick={() => handleAddOption(card.id)} className="mt-3 text-indigo-600 dark:text-indigo-400 text-xs font-bold hover:underline flex items-center gap-1 px-1"><Plus size={14} /> {t('create_set.add_option')}</button>)}</div>
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="bg-blue-50 dark:bg-blue-900/10 p-4 rounded-xl border border-blue-100 dark:border-blue-800/30"><label className="block text-xs font-bold text-blue-700 dark:text-blue-400 uppercase mb-2 flex items-center gap-1"><Lightbulb size={14} /> {t('create_set.explanation_label')}</label><textarea className="w-full p-2 bg-transparent border-none focus:ring-0 outline-none text-blue-900 dark:text-blue-200 placeholder-blue-300 resize-none text-sm leading-relaxed" placeholder={t('create_set.explanation_ph')} rows={2} value={card.explanation || ''} onChange={e => handleExplanationChange(card.id, e.target.value)}/></div><div className="bg-gray-100 dark:bg-gray-700/30 p-4 rounded-xl border border-gray-200 dark:border-gray-700"><label className="block text-xs font-bold text-gray-600 dark:text-gray-400 uppercase mb-2 flex items-center gap-1"><Link size={14} /> {t('create_set.link_label')}</label><input type="text" className="w-full p-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm text-gray-800 dark:text-white placeholder-gray-400" placeholder="https://..." value={card.relatedLink || ''} onChange={e => handleLinkChange(card.id, e.target.value)}/></div></div>
                              </div>
                          </div>
                          ))}
                          <button onClick={handleAddCard} className="w-full py-4 mt-4 bg-gray-100 dark:bg-gray-800/50 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl text-gray-500 dark:text-gray-400 font-bold hover:border-indigo-500 dark:hover:border-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all flex items-center justify-center gap-2 shadow-sm"><Plus size={20} /> {t('create_set.add_new_card')}</button>
                        </div>
                    ) : (
                        /* Text Editor UI ... */
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-[calc(100vh-250px)]">
                            <div className="flex flex-col h-full bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden"><div className="p-3 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center shrink-0"><span className="text-xs font-bold uppercase text-gray-500 dark:text-gray-400">{t('create_set.input_label')}</span><div className="text-[10px] text-gray-400 flex gap-2"><span>{t('create_set.input_hint_correct')}</span><span>{t('create_set.input_hint_break')}</span></div></div><textarea className="flex-1 w-full p-4 bg-transparent outline-none font-mono text-sm leading-relaxed text-gray-800 dark:text-gray-200 h-full resize-none" placeholder={`Nhập câu hỏi tại đây...`} value={textEditorContent} onChange={(e) => setTextEditorContent(e.target.value)}/></div>
                            <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden"><div className="p-3 bg-100 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600 flex justify-between items-center shrink-0"><span className="text-xs font-bold uppercase text-gray-500 dark:text-gray-300 flex items-center gap-2"><Eye size={12} /> {t('create_set.preview_title')} ({parsedPreviewCards.length})</span></div><div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-white dark:bg-gray-800/30">{parsedPreviewCards.length === 0 ? (<div className="text-center text-gray-400 text-sm mt-10">{t('create_set.preview_empty')}</div>) : (parsedPreviewCards.map((card, idx) => { const isValid = card.term && card.definition && card.options?.length && card.options.length >= 2; return (<div key={idx} className={`p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm border ${isValid ? 'border-gray-200 dark:border-gray-700' : 'border-orange-300 dark:border-orange-500'}`}><div className="flex gap-2 items-start mb-2"><span className="text-xs font-bold bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded">{idx + 1}</span><p className="text-sm font-bold text-gray-900 dark:text-white flex-1 whitespace-pre-line">{card.term}</p>{!isValid && <AlertTriangle size={14} className="text-orange-500" />}</div><div className="space-y-1 ml-6">{card.options?.map((opt, i) => (<div key={i} className={`text-xs flex items-center gap-2 ${opt === card.definition ? 'text-green-600 dark:text-green-400 font-bold' : 'text-gray-500 dark:text-gray-400'}`}>{opt === card.definition && <Check size={12} />}<span className="whitespace-pre-line">{opt}</span></div>))}</div></div>);}))}</div></div>
                        </div>
                    )}
                </div>
            </div>
        </div>
        {/* ... Guide & AI Modal components ... */}
      </div>
    )
  );
};
