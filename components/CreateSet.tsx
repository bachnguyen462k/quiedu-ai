...
const CreateSet: React.FC<CreateSetProps> = ({ onSave, onCancel, onGoToAiTextbook, history = [], onSelectHistory }) => {
      ...
      return (
          <div className="max-w-6xl mx-auto px-4 py-12 animate-fade-in relative pb-32">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
                  <button onClick={() => { setCreationStep('EDITOR'); setEditingSetId(null); setCreationSource('MANUAL'); setStatus('ACTIVE'); setTitle(''); setCards([{id:uuidv4(),term:'',definition:'',options:['','','',''],explanation:'',relatedLink:''}])}} disabled={!canManualEntry} className={`group bg-white dark:bg-gray-800 p-5 rounded-xl border transition-all text-left flex flex-col h-full relative ${canManualEntry ? 'hover:border-blue-500 hover:shadow-xl' : 'opacity-50 grayscale'}`}>
                      <div className="w-12 h-12 bg-blue-100 dark:bg-blue-900/30 text-blue-600 rounded-xl flex items-center justify-center mb-4"><Keyboard size={24} /></div>
                  ...
                  <button onClick={() => { setAiMode('TEXT_TOPIC'); setShowAiModal(true); }} disabled={!canAiTopic} className={`group bg-white dark:bg-gray-800 p-5 rounded-xl border transition-all text-left flex flex-col h-full relative ${canAiTopic ? 'hover:border-purple-500 hover:shadow-xl' : 'opacity-50 grayscale'}`}>
                      <div className="w-12 h-12 bg-purple-100 dark:bg-purple-900/30 text-purple-600 rounded-xl flex items-center justify-center mb-4"><BrainCircuit size={24} /></div>
                  ...
                  <button onClick={() => { setAiMode('FILE_SCAN_QUIZ'); setShowAiModal(true); }} disabled={!canScanDoc} className={`group bg-white dark:bg-gray-800 p-5 rounded-xl border transition-all text-left flex flex-col h-full relative ${canScanDoc ? 'hover:border-indigo-500 hover:shadow-xl' : 'opacity-50 grayscale'}`}>
                      <div className="w-12 h-12 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 rounded-xl flex items-center justify-center mb-4"><ScanLine size={24} /></div>
                  ...
                  <button onClick={onGoToAiTextbook} disabled={!canAiPlanner} className={`group bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-gray-800 dark:to-gray-800 p-5 rounded-xl border transition-all text-left flex flex-col h-full relative ${canAiPlanner ? 'hover:border-pink-500 hover:shadow-xl' : 'opacity-50 grayscale'}`}>
                      <div className="w-12 h-12 bg-pink-100 dark:bg-pink-900/30 text-pink-600 rounded-xl flex items-center justify-center mb-4"><BookOpen size={24} /></div>
              ...
              <div className="max-w-6xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-sm border overflow-hidden">
                  <div className="px-6 py-4 border-b bg-gray-50/50 dark:bg-gray-700/50 flex items-center justify-between">
                              ...
                              {serverSets.map((r) => (
                                  <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                      ...
                                      <td className="px-6 py-4">
                                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${r.status === 'DRAFT' ? 'bg-gray-100 text-gray-800' : 'bg-green-100 text-green-800'}`}>{r.status || 'ACTIVE'}</span>
                                      </td>
                                      <td className="px-6 py-4 text-right">
                                          <button onClick={() => handleSelectServerSet(r.id)} className="text-indigo-600 font-medium text-xs border px-3 py-1.5 rounded-lg hover:bg-indigo-50">Xem lại</button>
                                      </td>
                                  </tr>
                              ))}
                          </tbody>
              ...
              {showAiModal && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg p-6 shadow-2xl">
                        ...
                        {aiMode === 'TEXT_TOPIC' ? (
                            <textarea className="w-full border dark:border-gray-600 bg-gray-50 dark:bg-gray-700 dark:text-white rounded-xl p-4 h-32 mb-4 outline-none" placeholder={t('create_set.ai_modal_topic_ph')} value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} />
                        ) : (
                            <div className="border-2 border-dashed rounded-xl p-8 text-center relative mb-6">
                                ...
                                {aiFile ? <div className="text-indigo-600 font-bold">{aiFile.name}</div> : <div className="text-gray-400"><Upload size={32} className="mx-auto mb-2" />{t('create_set.click_upload')}</div>}
                            </div>
                        )}
                        <div className="flex justify-end gap-3 pt-2">
                            <button onClick={() => setShowAiModal(false)} className="px-5 py-2.5 text-gray-500 font-medium">Hủy</button>
  ...
  return (
    <div className="w-full h-[calc(100vh-80px)] flex flex-col animate-fade-in bg-gray-50 dark:bg-gray-900 overflow-hidden">
      <div className="flex justify-between items-center px-6 py-3 bg-white dark:bg-gray-800 z-30 border-b shrink-0">
        <div className="flex items-center gap-3">
            <button onClick={() => setCreationStep('MENU')} className="p-2 rounded-full hover:bg-gray-100 text-gray-500"><ArrowLeft size={20} /></button>
            <h2 className="text-xl font-bold dark:text-white">{editingSetId ? "Chỉnh sửa" : t('create_set.editor_title')}</h2>
        </div>
        <div className="flex gap-3">
            <button onClick={onCancel} className="px-4 py-2 text-gray-500 font-medium">Hủy</button>
            <button onClick={handleSave} disabled={isSaving} className="px-6 py-2 bg-indigo-600 text-white rounded-lg font-medium flex items-center gap-2">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          <div className="mx-auto w-full max-w-[1920px] grid grid-cols-1 lg:grid-cols-4 gap-8">
              <div className="lg:col-span-1 space-y-6">
                  <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border">
                      ...
                      <div className="space-y-4">
                          <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Tiêu đề</label><input type="text" className="w-full p-2 bg-gray-50 dark:bg-gray-700 dark:text-white border rounded-lg outline-none font-bold" value={title} onChange={e => setTitle(e.target.value)} /></div>
                          <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Mô tả</label><textarea className="w-full p-2 bg-gray-50 dark:bg-gray-700 dark:text-white border rounded-lg outline-none resize-none" rows={2} value={description} onChange={e => setDescription(e.target.value)} /></div>
                          <div className="grid grid-cols-2 gap-3">
                              <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Quyền</label><select value={privacy} onChange={e => setPrivacy(e.target.value as any)} className="w-full p-2 bg-gray-50 dark:bg-gray-700 dark:text-white border rounded-lg text-sm outline-none"><option value="PUBLIC">Công khai</option><option value="PRIVATE">Riêng tư</option></select></div>
                              <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Trạng thái</label><select value={status} onChange={e => setStatus(e.target.value)} className="w-full p-2 bg-gray-50 dark:bg-gray-700 dark:text-white border rounded-lg text-sm outline-none"><option value="ACTIVE">Hoạt động</option><option value="DRAFT">Bản nháp</option></select></div>
                          </div>
                          <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Cấp học</label><select value={level} onChange={e => setLevel(e.target.value)} className="w-full p-2 bg-gray-50 dark:bg-gray-700 dark:text-white border rounded-lg text-sm outline-none">{EDUCATION_LEVELS.map(l=><option key={l} value={l}>{l}</option>)}</select></div>
                          <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Môn học</label><input type="text" className="w-full p-2 bg-gray-50 dark:bg-gray-700 dark:text-white border rounded-lg text-sm outline-none" value={subject} onChange={e => setSubject(e.target.value)} /></div>
                      </div>
                  </div>
                  <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border flex flex-col max-h-[400px]">
                      ...
                      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-1">
                          {cards.map((c, i) => (
                              <button key={c.id} onClick={() => scrollToCard(c.id)} className={`w-full text-left px-3 py-2 rounded-lg text-sm truncate border-l-2 ${activeCardId === c.id ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 border-l-indigo-600 font-medium' : 'text-gray-500 hover:bg-gray-50 border-l-transparent'}`}>
              ...
              <div className="lg:col-span-3 space-y-6">
                  <div className="flex justify-between items-center bg-white dark:bg-gray-800 p-4 rounded-xl border">
                      <div className="flex gap-4">
                          <button onClick={() => handleSwitchMode('VISUAL')} className={`px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2 ${editorMode === 'VISUAL' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:bg-gray-100'}`}><Layers size={16} /> Giao diện thẻ</button>
                          <button onClick={() => handleSwitchMode('TEXT')} className={`px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2 ${editorMode === 'TEXT' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:bg-gray-100'}`}><FileText size={16} /> Nhập văn bản</button>
                          <button onClick={() => setShowTextGuide(true)} className="flex items-center gap-1 text-gray-500 hover:text-indigo-600 text-sm font-medium transition-colors"><HelpCircle size={16} /> Hướng dẫn nhập</button>
                      </div>
                  </div>
                  {editorMode === 'VISUAL' ? (
                      <div className="space-y-6 pb-20">
                        {cards.map((card, index) => (
                        <div key={card.id} ref={el => { cardRefs.current[card.id] = el; }} className={`bg-white dark:bg-gray-800 rounded-xl border p-6 transition-all ${activeCardId === card.id ? 'border-indigo-500 ring-1 ring-indigo-200 shadow-md' : 'hover:border-gray-300'}`} onClick={() => setActiveCardId(card.id)}>
                            <div className="flex justify-between items-center mb-4 pb-4 border-b">
                                <div className="flex items-center gap-3"><span className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center font-bold text-sm">{index+1}</span><span className="text-xs font-bold uppercase text-indigo-600">QUIZ</span></div>
                                <button onClick={(e) => { e.stopPropagation(); setCards(cards.filter(c=>c.id!==card.id)); }} className="text-gray-400 hover:text-red-500"><Trash2 size={18} /></button>
                            </div>
                            <div className="space-y-4">
                                <div><label className="text-xs font-bold text-gray-500 uppercase block mb-1">Câu hỏi</label><textarea className="w-full p-4 rounded-xl bg-gray-50 dark:bg-gray-700 dark:text-white border outline-none font-medium text-lg" rows={2} value={card.term} onChange={e=>setCards(cards.map(c=>c.id===card.id?{...c,term:e.target.value}:c))} /></div>
                                <div><label className="text-xs font-bold text-gray-500 uppercase block mb-1">Các lựa chọn</label><div className="grid md:grid-cols-2 gap-4">
                                    {card.options?.map((opt, oi) => {
                                        const isCorrect = card.definition === opt && opt !== '';
                                        return (
                                            <div key={oi} className={`flex items-center gap-3 p-3 rounded-xl border ${isCorrect ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : 'bg-white dark:bg-gray-850'}`}>
                                                <div onClick={() => setCards(cards.map(c=>c.id===card.id?{...c,definition:opt}:c))} className={`w-6 h-6 rounded-full border-2 flex items-center justify-center cursor-pointer ${isCorrect ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300'}`}><Check size={14} /></div>
                                                ...
                                                <button onClick={()=> {const next=[...(card.options||[])]; next.splice(oi,1); setCards(cards.map(c=>c.id===card.id?{...c,options:next,definition:card.definition===opt?'':card.definition}:c));}} className="text-gray-300 hover:text-red-500"><X size={14} /></button>
                                            </div>
                                        );
                                    })}
                                </div><button onClick={()=>setCards(cards.map(c=>c.id===card.id?{...c,options:[...(c.options||[]),'']}:c))} className="mt-2 text-indigo-600 text-xs font-bold">+ Thêm lựa chọn</button></div>
                            </div>
                        </div>
                        ))}
                        <button onClick={handleAddCard} className="w-full py-4 bg-gray-100 dark:bg-gray-800 border-2 border-dashed rounded-xl text-gray-500 font-bold hover:border-indigo-500 flex items-center justify-center gap-2"><Plus size={20} /> Thêm câu hỏi mới</button>
                      </div>
                  ) : (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-[calc(100vh-250px)]">
                          <textarea className="bg-white dark:bg-gray-800 border rounded-xl p-4 font-mono text-sm outline-none dark:text-white" placeholder="Nhập câu hỏi theo định dạng..." value={textEditorContent} onChange={e=>setTextEditorContent(e.target.value)} />
                          <div className="bg-gray-50 dark:bg-gray-900 border rounded-xl overflow-hidden flex flex-col">
                              <div className="p-3 border-b font-bold text-xs uppercase text-gray-500 dark:text-gray-400">Xem trước ({parsedPreviewCards.length})</div>
                              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                  {parsedPreviewCards.map((p, i) => (
                                      <div key={i} className="p-3 bg-white dark:bg-gray-800 rounded border shadow-sm">
                                          <div className="font-bold text-sm dark:text-white">{i+1}. {p.term}</div>
                                          <div className="mt-2 space-y-1">{p.options?.map((o,oi)=><div key={oi} className={`text-xs ${o===p.definition?'text-green-600 font-bold':'text-gray-500'}`}>- {o}</div>)}</div>
                                      </div>
                                  ))}
                              </div>
                          </div>
      ...
      {showTextGuide && (
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
            <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg shadow-2xl border overflow-hidden">
                <div className="p-5 border-b flex justify-between items-center bg-gray-50 dark:bg-gray-700">
                    <h3 className="font-bold flex items-center gap-2 dark:text-white"><Info size={20} className="text-indigo-600" /> Hướng dẫn nhập văn bản</h3>
                    <button onClick={() => setShowTextGuide(false)} className="text-gray-400"><X size={24} /></button>
                </div>
                <div className="p-6 space-y-4 text-sm text-gray-600 dark:text-gray-300">
                    ...
                    <div className="bg-gray-900 p-4 rounded-lg relative group">
                        <pre className="text-xs text-green-400 font-mono">{SAMPLE_TEXT_FORMAT}</pre>
                        <button onClick={() => { setTextEditorContent(SAMPLE_TEXT_FORMAT); setShowTextGuide(false); }} className="absolute top-2 right-2 bg-indigo-600 text-white p-1 rounded text-[10px] font-bold">Copy mẫu</button>
                    </div>
                </div>
                <div className="p-4 border-t flex justify-end">
                    <button onClick={() => setShowTextGuide(false)} className="bg-gray-100 px-4 py-2 rounded-lg font-bold text-sm">Đã hiểu</button>
                </div>
            </div>
        </div>
      )}
