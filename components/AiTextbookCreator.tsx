            ...
            <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="bg-gray-100 p-1 rounded-lg flex font-medium text-sm">
                    <button 
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-md transition-all ${activeTab === 'CREATE' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
                    >
                        <Upload size={16} /> Tạo mới
                    </button>
                    <button 
                         onClick={() => setActiveTab('HISTORY')}
                         className={`flex items-center gap-2 px-4 py-2 rounded-md transition-all ${activeTab === 'HISTORY' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
                    >
                        <History size={16} /> Lịch sử đã soạn ({history.length})
                    </button>
                </div>
            </div>
                ...
                <div className="animate-fade-in">
                    {history.length === 0 ? (
                        <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-300">
                             ...
                             <h3 className="text-lg font-bold text-gray-900 mb-2">Chưa có bài soạn nào</h3>
                             <p className="text-gray-500 mb-6">Các bài học do AI tạo ra sẽ được lưu tự động tại đây.</p>
                             <button 
                                onClick={() => setActiveTab('CREATE')}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {history.map(record => (
                                <div key={record.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md hover:border-indigo-300 transition-all group">
                                    ...
                                    <h3 className="font-bold text-gray-900 text-lg mb-1 line-clamp-1">{record.result.topics[0]?.topicName || "Không có tên"}</h3>
                                    <p className="text-sm text-gray-500 mb-4">{record.result.subject} - {record.result.grade}</p>
                                    ...
                                    <button 
                                        ...
                                        className="w-full py-2 bg-indigo-50 text-indigo-600 rounded-lg font-bold text-sm group-hover:bg-indigo-600 group-hover:text-white transition-colors flex items-center justify-center gap-2"
                                    >
                        ...
                        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200 mb-8">
                            <div className="flex flex-col md:flex-row gap-6 items-center">
                                <div className="flex-1 w-full">
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Tải lên tài liệu (PDF hoặc Ảnh)</label>
                                    <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:bg-gray-50 transition-colors relative group overflow-hidden">
                                        <input 
                                            type="file" 
                                        ...
                                        {file ? (
                                            <div className="flex flex-col items-center relative z-10">
                                                {file.type.startsWith('image/') ? (
                                                    <div className="w-32 h-32 mb-3 rounded-lg overflow-hidden border border-gray-200 shadow-sm">
                                                ...
                                                <button className="mt-4 text-xs text-gray-500 underline hover:text-indigo-600 z-30 relative cursor-pointer pointer-events-none group-hover:pointer-events-auto">
                                            ...
                                            <div>
                                                <div className="flex justify-center gap-4 mb-3">
                                                    <div className="w-12 h-12 bg-gray-100 text-gray-400 rounded-full flex items-center justify-center group-hover:bg-indigo-50 group-hover:text-indigo-500 transition-colors">
                                                        <Upload size={24} />
                                                    </div>
                                                </div>
                                                <p className="font-medium text-gray-700">Nhấn để tải lên file</p>
                                                <p className="text-xs text-gray-400 mt-1">Hỗ trợ: PDF, JPG, PNG (Tối đa 10MB)</p>
                                                <div className="flex justify-center gap-2 mt-2">
                                                    <span className="bg-red-50 text-red-600 text-[10px] font-bold px-2 py-0.5 rounded flex items-center gap-1"><FileType size={10} /> PDF</span>
                                                    <span className="bg-blue-50 text-blue-600 text-[10px] font-bold px-2 py-0.5 rounded flex items-center gap-1"><ImageIcon size={10} /> ẢNH</span>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                        ...
                        <div className="animate-fade-in grid grid-cols-1 lg:grid-cols-4 gap-8">
                            ...
                            <div className="lg:col-span-1 space-y-4">
                                <button 
                                    ...
                                    className="w-full mb-2 bg-gray-100 text-gray-600 py-2 rounded-lg font-bold text-sm hover:bg-gray-200"
                                >
                                    &larr; Tạo bài mới
                                </button>
            ...
            {showSaveModal && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl">
                        ...
                        <div className="p-6 border-b border-gray-200 flex justify-between items-center bg-gray-50 rounded-t-2xl">
                            <div>
                                <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                                    <Edit3 size={20} className="text-indigo-600" /> Xem lại & Chỉnh sửa
                                </h3>
                                <p className="text-sm text-gray-500 mt-1">Điều chỉnh nội dung trước khi lưu vào thư viện.</p>
                            </div>
                            <button onClick={() => setShowSaveModal(false)} className="text-gray-400 hover:text-gray-900">
                                <X size={24} />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50/50">
                            ...
                            <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm space-y-4">
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-lg"
                                    />
                                </div>
                                <div>
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center">
                                    <h4 className="font-bold text-gray-800">Danh sách câu hỏi ({editingSet.cards.length})</h4>
                                    <button 
                                ...
                                {editingSet.cards.map((card, idx) => (
                                    <div key={card.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm group">
                                        ...
                                        <div className="grid md:grid-cols-2 gap-4">
                                            <div>
                                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Câu hỏi / Thuật ngữ</label>
                                                <textarea
                                                    rows={2}
                                                    value={card.term}
                                                    onChange={(e) => {
                                                    ...
                                                    className="w-full p-2 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none resize-none text-sm font-medium"
                                                />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Đáp án / Định nghĩa</label>
                                                <textarea
                                                    rows={2}
                                                    ...
                                                    className="w-full p-2 bg-gray-50 border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-green-500 outline-none resize-none text-sm text-gray-700"
                                                />
                                            </div>
                                        </div>
                                    </div>
                        ...
                        {/* Footer */}
                        <div className="p-6 border-t border-gray-200 bg-white rounded-b-2xl flex justify-end gap-3">
                            <button 
