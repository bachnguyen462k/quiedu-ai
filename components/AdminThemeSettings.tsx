...
import { Snowflake, Flame, Leaf, Globe, CheckCircle2, AlertCircle, Loader2 } from 'lucide-react';
...
const AdminThemeSettings: React.FC = () => {
    ...
    const handleThemeSelect = async (id: EventTheme) => {
        ...
        setIsSaving(id);
        try {
            // Call API PUT /setting/event/{mode}
            await settingEventService.updateGlobalEventTheme(id);
        ...
        <div className="max-w-4xl mx-auto px-6 py-12 animate-fade-in pb-32">
            <div className="mb-10">
                ...
                <p className="text-gray-500 dark:text-gray-400 font-medium">
                    Chọn loại chủ đề trang trí sẽ hiển thị khi sự kiện được bật trên toàn hệ thống.
                </p>
            </div>
            ...
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 p-4 rounded-2xl flex items-start gap-4 mb-10 transition-colors">
                ...
                <p className="text-sm text-yellow-800 dark:text-yellow-200 leading-relaxed font-medium">
                    Việc thay đổi chủ đề tại đây sẽ gửi yêu cầu <strong>PUT /setting/event/{'{mode}'}</strong> lên máy chủ. Cài đặt này có hiệu lực ngay lập tức cho toàn bộ người dùng khi họ bật hiệu ứng.
                </p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {themes.map((t) => {
                    const isActive = eventTheme === t.id;
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className={`w-full h-1.5 rounded-full bg-gray-100 dark:bg-gray-700 overflow-hidden`}>
                                 <div className={`h-full ${t.color} transition-all duration-500 ${isActive ? 'w-full' : 'w-0'}`}></div>
                            </div>
                        </div>
                    );
                })}
