...
const AdminThemeSettings: React.FC = () => {
    ...
    const handleThemeSelect = (id: EventTheme) => {
        setEventTheme(id);
        addNotification(`Đã đổi chủ đề sự kiện thành: ${id}`, 'success');
    };
        ...
        <div className="max-w-4xl mx-auto px-6 py-12 animate-fade-in pb-32">
            ...
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 p-4 rounded-2xl flex items-start gap-4 mb-10 transition-colors">
                ...
                <p className="text-sm text-yellow-800 dark:text-yellow-200 leading-relaxed font-medium">
                    <strong>Lưu ý:</strong> Việc thay đổi chủ đề tại đây sẽ có hiệu lực ngay lập tức cho tất cả các trang. Một số chủ đề có thể bao gồm hiệu ứng hoạt họa (tuyết rơi, lá rụng) có thể ảnh hưởng nhẹ đến hiệu năng trên các thiết bị cũ.
                </p>
            </div>
                ...
                {themes.map((t) => (
                    <div 
                        ...
                        className={`group relative p-6 rounded-[32px] border-2 transition-all cursor-pointer overflow-hidden ${
                            ...
                            : 'border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-850 hover:border-gray-300 dark:hover:border-gray-700'
                        }`}
                    >
                        {eventTheme === t.id && (
            ...
            <div className="mt-16 p-8 rounded-[40px] bg-gradient-to-br from-indigo-600 to-purple-700 text-white shadow-2xl relative overflow-hidden">
                <div className="relative z-10">
                    ...
                    <p className="text-indigo-100 font-medium mb-6 max-w-lg">
                        Khi chọn Giáng sinh hoặc Tết, hệ thống sẽ tự động thêm các layer trang trí tinh tế ở các góc màn hình để không làm ảnh hưởng đến trải nghiệm học tập của người dùng.
                    </p>
