...
const SetDetailView: React.FC<SetDetailViewProps> = ({ set: metadata, onBack, onStartFlashcard, onStartQuiz, onToggleFavorite: localToggle }) => {
  ...
  const fetchComments = async (page: number, refresh: boolean = false) => {
    if (!metadata.id || commentsLoading) return;
  ...
  const handlePostComment = async (e?: React.FormEvent | React.MouseEvent | React.KeyboardEvent) => {
    ...
    try {
        ...
        if (response.code === 1000) {
            setNewComment('');
            // Thêm trực tiếp vào danh sách local để hiển thị ngay lập tức
            setComments(prev => [...prev, response.result]);
            setTotalComments(prev => prev + 1);
            
            // Cuộn mượt xuống cuối danh sách bình luận (chỉ trong ô scroll)
            setTimeout(() => {
  ...
  if (isLoading || !preview) {
      return (
          <div className="min-h-[60vh] flex flex-col items-center justify-center py-20">
              ...
              <p className="text-gray-400 font-black uppercase text-xs tracking-widest">Đang tải...</p>
          </div>
      );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-8 animate-fade-in transition-colors">
      <div className="flex justify-between items-center gap-4 mb-8">
        <button type="button" onClick={onBack} className="flex items-center gap-2 text-gray-500 hover:text-brand-blue font-black uppercase text-[10px] md:text-xs tracking-widest transition-colors"><ArrowLeft size={18} /> {t('set_detail.back_library')}</button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10 items-start">
        <div className="lg:col-span-2 space-y-8">
          ...
          {/* Combined Comments & Info with Independent Scroll */}
          <div className="bg-white dark:bg-gray-855 rounded-[40px] border border-gray-100 dark:border-gray-800 transition-colors shadow-sm overflow-hidden flex flex-col h-[700px]">
              {/* Header - Fixed */}
              ...
              {/* Scrollable Area - This is where the magic happens */}
              <div 
                ref={commentListRef}
                className="flex-1 overflow-y-auto p-6 md:px-10 space-y-10 custom-scrollbar bg-white dark:bg-gray-855"
              >
                  {/* Additional context info inside scroll area */}
                  <div className="bg-indigo-50 dark:bg-indigo-900/10 p-6 rounded-3xl border border-indigo-100 dark:border-indigo-900/30 transition-colors mb-8">
                              ...
                              <div key={comment.id} className="flex gap-4 group items-start animate-fade-in">
                                  ...
                                  <div className="flex-1">
                                      <div className="inline-block bg-gray-100 dark:bg-gray-800 px-5 py-3 rounded-[24px] max-w-full shadow-sm">
                                          <div className="font-black text-gray-900 dark:text-white text-xs mb-1 hover:underline cursor-pointer">{comment.userId}</div>
                                          <div className="text-sm text-gray-800 dark:text-gray-200 leading-relaxed font-medium">{comment.content}</div>
                                      </div>
              ...
              {/* Sticky Footer - Always at bottom of this container */}
              <div className="sticky bottom-0 bg-white dark:bg-gray-855 border-t border-gray-100 dark:border-gray-800 p-6 md:px-10 transition-all z-30 shrink-0 shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
                    <div className="flex gap-3 items-end">
                        ...
                        <div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-[28px] px-5 py-2.5 flex flex-col focus-within:ring-2 focus-within:ring-brand-blue/20 focus-within:bg-white dark:focus-within:bg-gray-750 transition-all border border-transparent">
                            ...
                            />
                            <div className="flex justify-between items-center mt-2 border-t border-gray-200/50 dark:border-gray-700/50 pt-2 pb-0.5">
                                ...
                                <button 
                                    ...
                                    className={`p-2 rounded-full transition-all ${newComment.trim() ? 'text-brand-blue hover:bg-blue-50 dark:hover:bg-blue-900/30' : 'text-gray-300 dark:text-gray-600'}`}
                                >
        ...
        <div className="space-y-6 lg:sticky lg:top-24">
            <div className="bg-white dark:bg-gray-855 p-6 md:p-8 rounded-[40px] shadow-xl md:shadow-2xl border border-brand-blue/10 dark:border-gray-800 transition-colors">
                ...
                <div className="mt-10 pt-8 border-t border-gray-50 dark:border-gray-800">
                    ...
                    <div className="flex gap-2">
                        <div onClick={() => { navigator.clipboard.writeText(shareCode); addNotification("Đã copy mã!", "success"); }} className="flex-1 bg-gray-50 dark:bg-gray-800 p-3 rounded-2xl text-center cursor-pointer border border-transparent hover:border-brand-blue transition-all group active:scale-95">
                            <span className="text-[8px] text-gray-400 font-black block mb-0.5 tracking-tighter">MÃ THAM GIA</span>
      ...
      {showQrModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-fade-in" onClick={() => setShowQrModal(false)}>
            <div className="bg-white dark:bg-gray-800 p-10 rounded-[40px] shadow-2xl max-w-sm w-full flex flex-col items-center relative" onClick={e => e.stopPropagation()}>
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm font-medium mb-8">Quét để truy cập nhanh trên Mobile</p>
                <div className="bg-white p-5 rounded-[40px] border-8 border-brand-blue/5 mb-8 overflow-hidden shadow-inner"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(shareUrl)}`} alt="QR" className="w-48 h-48" /></div>
