                ...
                <>
                    ...
                    {clientConfigError ? (
                        <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl flex items-start gap-3 mb-4 text-xs text-red-600 dark:text-red-400 font-medium">
                            ...
                            <p>Thiếu GOOGLE_CLIENT_ID. Vui lòng kiểm tra lại biến môi trường.</p>
                        </div>
                    ) : (
                        <div ref={googleButtonRef} className="w-full min-h-[50px] mb-4 overflow-hidden flex justify-center"></div>
                    )}
                    
                    {isGoogleSubmitting && (
                        <div className="flex items-center justify-center gap-2 text-sm text-brand-blue font-bold">
                            <ThemeLoader size={18} /> {t('quiz.generating')}
                        </div>
                    )}
                </>
            )}
